...
export async function processPipeline(projectId: string) {
  ...
  try {
    ...
    if (!project) {
      logPipeline(projectId, "Error: Project not found in store.");
      return;
    }

    if (project.status !== "uploading" && project.status !== "failed") {
      logPipeline(projectId, `Pipeline already in progress (Status: ${project.status}). Aborting run.`);
      return;
    }

    // 1. Preparation (Download)
    const videoPath = await prepareVideo(projectId, workDir, project.videoPath);
    ...
    // 5. Dubbing
    const dubbedAudioPath = await performDubbing(projectId, workDir, translatedSubtitles, project.targetLanguage, audioPath);
...
async function prepareVideo(projectId: string, workDir: string, videoPath: string | undefined): Promise<string> {
  if (videoPath?.startsWith("youtube:")) {
    ...
    // Using yt-dlp via exec as it's a specific CLI tool
    const ytdlpCommand = `yt-dlp -f "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]" --extractor-args "youtube:player_client=android,ios" -o "${outPath}" "${url}" --max-filesize 500m`;
    await execPromise(ytdlpCommand, { timeout: 5 * 60 * 1000 });
    
    updateProject(projectId, { videoPath: outPath });
...
async function extractAudio(projectId: string, workDir: string, videoPath: string): Promise<string> {
  ...
  updateProject(projectId, { audioPath });
  logPipeline(projectId, "Audio extraction successful.");
  return audioPath;
}

async function performTranscription(projectId: string, audioPath: string): Promise<Subtitle[]> {
  logPipeline(projectId, "Starting transcription...");
  const audioBuffer = readFileSync(audioPath);
  const segments = await transcribeAudio(audioBuffer);
  
  const subtitles: Subtitle[] = segments.map((seg: any) => ({
    id: uuidv4(),
    start: seg.start,
    end: seg.end,
    text: seg.text.trim(),
  ...
  updateProject(projectId, { subtitles });
  return subtitles;
}

async function performTranslation(projectId: string, project: any, subtitles: Subtitle[]): Promise<Subtitle[]> {
  updateProject(projectId, { status: "translating" });
  logPipeline(projectId, "Starting translation...");
  
  const mapping = await translateText(subtitles, project.sourceLanguage, project.targetLanguage);
  
  const updatedSubtitles: Subtitle[] = subtitles.map((sub, i) => ({
    ...sub,
    translatedText: mapping[i + 1] || sub.text,
  }));
  
  updateProject(projectId, { subtitles: updatedSubtitles });
...
async function performDubbing(
  ...
  updateProject(projectId, { status: "dubbing" });
  ...
  // Mix logic: bg (input 0) at volume 0, dubbed voices at dynamic boost
  ...
  updateProject(projectId, { dubbedAudioPath });
...
async function muxFinalVideo(projectId: string, workDir: string, videoPath: string, dubbedAudioPath: string): Promise<string> {
  ...
  updateProject(projectId, { finalVideoPath, status: "completed" });
