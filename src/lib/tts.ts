...
export async function generateTTSSegment(
    ...
    try {
        const result = spawnSync(pythonPath, [bridgePath, text, outputPath, voice]);
        ...
        logPipeline(projectId, `Edge-TTS Failed at segment ${index}: ${e.message}`);
        throw new Error(`Edge-TTS Error: ${e.message || "Unknown error"}`);
    }
}
