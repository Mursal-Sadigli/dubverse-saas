...
import { 
  Play, Calendar, Languages, MoreVertical, ExternalLink,
  Clock, CheckCircle2, AlertCircle, Trash2, RefreshCw
} from "lucide-react";
...
const statusConfig = {
  pending: { icon: Clock, class: "bg-amber-500/10 text-amber-500 border-amber-500/20", label: "Gözləyir", pulse: true },
  uploading: { icon: Clock, class: "bg-blue-500/10 text-blue-500 border-blue-500/20", label: "Yüklənir", pulse: true },
  transcribing: { icon: Clock, class: "bg-violet-500/10 text-violet-500 border-violet-500/20", label: "Transkripsiya", pulse: true },
  translating: { icon: Languages, class: "bg-indigo-500/10 text-indigo-500 border-indigo-500/20", label: "Tərcümə", pulse: true },
  dubbing: { icon: Play, class: "bg-fuchsia-500/10 text-fuchsia-500 border-fuchsia-500/20", label: "Dublaj", pulse: true },
  completed: { icon: CheckCircle2, class: "bg-emerald-500/10 text-emerald-500 border-emerald-500/20", label: "Tamamlandı", pulse: false },
      ...
      <CardContent className="p-0 relative aspect-video bg-zinc-900 overflow-hidden">
        ...
        {isProcessing && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/50 backdrop-blur-[2px] gap-2">
            ...
            <span className="text-xs font-semibold text-white/80 animate-pulse">{status.label}...</span>
          </div>
        )}
            ...
            <DropdownMenuContent align="end" className="glass border-none shadow-xl rounded-xl p-1 min-w-[160px]">
              <DropdownMenuItem className="rounded-lg cursor-pointer font-medium" asChild>
                <Link href={`/projects/${project.id}`} className="flex items-center gap-2">
                  <ExternalLink className="size-4" /> Layihəni aç
                </Link>
              </DropdownMenuItem>
              {project.status === "failed" && onRetry && (
                ...
                >
                  <RefreshCw className="size-4 mr-2" /> Yenidən cəhd et
                </DropdownMenuItem>
              )}
              {onDelete && (
                <>
                  ...
                  >
                    <Trash2 className="size-4 mr-2" /> Layihəni sil
                  </DropdownMenuItem>
                </>
              )}
            </DropdownMenuContent>
